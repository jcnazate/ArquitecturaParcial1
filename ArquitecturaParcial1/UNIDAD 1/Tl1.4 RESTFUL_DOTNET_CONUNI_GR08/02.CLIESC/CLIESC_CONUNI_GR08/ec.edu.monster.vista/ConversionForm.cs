    ...
    public partial class ConversionForm : Form
    {
        ...
        private async void btnConvertir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Realizar conversión de forma asíncrona con timeout
                string tipoConversion = cmbTipoConversion.SelectedItem.ToString();
                _conversionModel = await RealizarConversionConTimeout(_conversionModel, tipoConversion);

                // Mostrar resultado
                if (_conversionModel.IsSuccess)
                {
                ...
                {
                    MessageBox.Show($"Error en la conversión: {_conversionModel.Message}", "Error de Conversión",
        ...
        private async Task<ConversionModel> RealizarConversionConTimeout(ConversionModel conversionModel, string tipoConversion)
        {
            try
            {
                // Crear una tarea con timeout de 10 segundos
                var timeoutTask = Task.Delay(10000); // 10 segundos
                var conversionTask = _conversionController.RealizarConversionAsync(conversionModel, tipoConversion);
                ...
                if (completedTask == timeoutTask)
                {
                    ...
                    conversionModel.Message = "Timeout de conexión";
            ...
            catch (Exception ex)
            {
                MessageBox.Show($"Error de conexión: {ex.Message}\n\nVerifique que el servicio REST esté ejecutándose en http://localhost:5000", "Error de Conexión",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                conversionModel.IsSuccess = false;
                conversionModel.Message = $"Error de conexión: {ex.Message}";
                return conversionModel;
            }
        }
